...
export async function* runAgent(
  ...
  yield { message: userMessage, type: "message" };
  ...
  for (let step = 0; step < maxSteps; step += 1) {
    ...
    if (deadline !== undefined && Date.now() > deadline) {
      yield { finishReason: "wall_clock", type: "turn_end" };
      return;
    }
    if (deps.tokenBudget !== undefined && usedTokens >= deps.tokenBudget) {
      yield { finishReason: "token_budget", type: "turn_end" };
      return;
    }
    ...
    try {
      ...
      if (!deps.fallbackModel) {
        yield { error: describeError(error), type: "error" };
        return;
      }
      try {
        turn = yield* streamModelTurn(
        ...
        yield { error: describeError(fallbackError), type: "error" };
        return;
      }
    }
    ...
    if (turn.calls.length === 0) {
      yield { finishReason: turn.finishReason, type: "turn_end" };
      return;
    }
    ...
    yield { message: toolMessage, type: "message" };
  }

  yield { finishReason: "max_steps", type: "turn_end" };
}
  ...
  for await (const part of stream.fullStream) {
    switch (part.type) {
      ...
      case "reasoning-delta": {
        yield { text: part.text, type: "reasoning" };
        break;
      }
      case "text-delta": {
