...
import type { AnyToolDef } from "@totvibe/core";
import { createBuiltinTools } from "./index";

function pick(tools: AnyToolDef[], name: string): AnyToolDef {
...
async function withSandboxDir<T>(
  run: (dir: string, tools: AnyToolDef[]) => Promise<T>,
): Promise<T> {
...
test("read, write, and list happy paths inside the sandbox", async () => {
  await withSandboxDir(async (dir, tools) => {
    ...
    await pick(tools, "write_file").execute(
      { path: "a.txt", content: "hello" },
      context,
    );
    expect(
      await pick(tools, "read_file").execute({ path: "a.txt" }, context),
    ).toBe("hello");
    expect(
      await pick(tools, "list_dir").execute({ path: "." }, context),
    ).toContain("a.txt");
  });
});
...
test("absolute paths outside the sandbox are rejected", async () => {
  await withSandboxDir(async (dir, tools) => {
    expect(
      pick(tools, "read_file").execute({ path: "/etc/hosts" }, { cwd: dir }),
...
test("a symlink that escapes the sandbox is rejected", async () => {
  await withSandboxDir(async (dir, tools) => {
    ...
    expect(
      pick(tools, "read_file").execute({ path: "link.txt" }, { cwd: dir }),
...
test("oversized output is truncated and spilled to a file", async () => {
  await withSandboxDir(async (dir, tools) => {
    ...
    const output = await pick(tools, "read_file").execute(
      { path: "big.txt" },
      context,
    );
